...
export function showFieldError(
  ...
  const errorId = field.id ? `${field.id}-error` : "";
  error.className = `form-field-error flex items-start gap-1.5 text-[11px] leading-tight font-semibold animate-fade-in ${
  ...
  icon.className =
    tone === "error"
      ? "self-center inline-flex h-4 w-4 shrink-0 items-center justify-center rounded-full border border-rose-300 bg-rose-100 text-[10px] font-black text-rose-700"
      : "self-center inline-flex h-4 w-4 shrink-0 items-center justify-center rounded-full border border-sky-300 bg-sky-100 text-[10px] font-black text-sky-700";
