...
export async function submitWeb3Form(
  ...
  const formData =
    formOrData instanceof FormData ? formOrData : new FormData(formOrData);
  const honeypotValue = String(formData.get('website') || '').trim();

  if (honeypotValue) {
    throw new Error('Submission blocked by anti-spam protection.');
  }
  ...
  if (!WEB3FORMS_ACCESS_KEY) {
    throw new Error('Form delivery is unavailable right now. Configure PUBLIC_WEB3FORMS_ACCESS_KEY and try again.');
  }
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      throw new Error('The secure form request timed out. Please check your connection and try again.');
    }
