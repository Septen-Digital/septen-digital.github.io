...
function bindCarlsCoffee(container: DemoContainer): void {
  ...
  items.forEach((item) => {
    item.addEventListener('click', () => {
      const isSelected = item.classList.contains('bg-[#29221E]') && item.classList.contains('border-[#E29578]');
  ...
  container.innerHTML = `
    <div class="bg-[#FAF6F0] min-h-screen text-[#4E342E] selection:bg-[#E3D5CA] text-left">
      ...
      <footer class="border-t border-[#3E2723]/10 bg-[#FAF6F0] py-8 text-xs text-center text-[#8D6E63]">
        <div class="max-w-4xl mx-auto px-4 space-y-2 font-mono">
          <p>© 2026 Carl’s Coffee. Durham medieval quarters. All rights reserved.</p>
          <p>Instagram: <span class="hover:text-amber-700 cursor-pointer">@carls_coffee_durham</span> • Tel: 0191 386 2000</p>
        </div>
      </footer>
    </div>
  `;
