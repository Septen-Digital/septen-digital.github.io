...
function bindPetersPlumbing(container: DemoContainer): void {
  ...
  container.querySelector('#peters-nav-cta')?.addEventListener('click', handleCta);
  container.querySelector('#peters-hero-cta')?.addEventListener('click', handleCta);
  container.querySelector('#peters-enquiry-form')?.addEventListener('submit', handleCta);
}
        ...
        <div class="max-w-5xl mx-auto px-6 grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
          ...
          <div class="md:col-span-6 space-y-6 text-left" data-reveal>
            ...
            <div class="flex flex-col sm:flex-row gap-3 pt-2">
              ...
              <a 
                href="#services"
                class="bg-transparent hover:bg-stone-50 text-[#2C5282] border-slate-300 border border-solid text-center font-bold text-xs py-3.5 px-6 rounded transition-all cursor-pointer tracking-wider"
              >
                Explore Services
              </a>
            </div>
            ...
            <div class="relative rounded-lg overflow-hidden shadow-sm aspect-video sm:aspect-4/3 bg-slate-100 hover-lift transition-transform duration-300">
              <img 
                ${responsiveImageAttrs({
                  src: '/demo-images/alder-and-pipe-plumbing/hero.webp',
                  baseWidth: 1100,
                  variantWidths: [480, 840],
                  sizes: '(min-width: 48rem) 50vw, 100vw',
                })}
                alt="Neat domestic tiling and plumbing in a modern residence" 
                loading="eager"
                fetchpriority="high"
                decoding="async"
                data-skeleton
                class="image-skeleton w-full h-full object-cover"
              />
      ...
      <section class="bg-[#FAFBFB] py-16 border-t border-stone-200 text-center">
        <div class="max-w-xl mx-auto px-6 space-y-6" data-reveal>
          ...
          <form id="peters-enquiry-form" class="space-y-3 max-w-sm mx-auto text-left">
            <div>
              <label class="text-[10px] font-mono uppercase tracking-widest text-slate-500 font-bold block leading-4 mb-1.5">Your Name</label>
              <input type="text" required placeholder="William Armstrong" class="w-full bg-white border border-stone-200 rounded p-2.5 text-xs text-slate-800 focus:outline-none focus:border-[#2C5282]" />
            </div>
            <div>
              <label class="text-[10px] font-mono uppercase tracking-widest text-slate-500 font-bold block leading-4 mb-1.5">Home Postcode</label>
              <input type="text" required placeholder="e.g. NE3 1AA" class="w-full bg-white border border-stone-200 rounded p-2.5 text-xs text-slate-800 focus:outline-none focus:border-[#2C5282]" />
            </div>
            <div>
              <label class="text-[10px] font-mono uppercase tracking-widest text-slate-500 font-bold block leading-4 mb-1.5">Request details</label>
              <textarea rows="3" placeholder="Describe your domestic plumbing plans or maintenance requirements..." class="w-full bg-white border border-stone-200 rounded p-2.5 text-xs text-slate-800 focus:outline-none focus:border-[#2C5282] resize-none font-sans"></textarea>
            </div>
