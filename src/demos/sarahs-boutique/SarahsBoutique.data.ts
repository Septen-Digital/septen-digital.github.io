...
export type SarahsBoutiqueCategory = (typeof sarahsBoutiqueCategories)[number];

// These IDs are shared between Astro markup and CSS hash states so anchors and
// no-JavaScript navigation stay in lockstep.
...
export const sarahsBoutiqueVisitConfirmationId = "visit-confirmation";
...
// These helpers deliberately keep anchor generation deterministic because the
// static hash navigation depends on the same IDs as the rendered markup.
export function getSarahsBoutiqueProductDetailId(productId: string): string {
  const product = sarahsBoutiqueInventory.find((item) => item.id === productId);
  const productSlug = product?.name.toLowerCase().replace(/[^a-z0-9]+/g, "-") ?? productId;
  return productSlug.replace(/^-|-$/g, "");
}

export function getSarahsBoutiqueCategoryAnchorId(category: SarahsBoutiqueCategory): string {
  return `catalogue-${category.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
}
...
export const sarahsBoutiqueInventory: SarahsBoutiqueProduct[] = [
  ...
  },
  {
    ...
    longDesc:
      "A core base piece for any cold day collection. We use spun premium wool fibres that do not bite. Extraordinarily fine, breathable, and designed to sit softly against skin with refined seamless wrist borders.",
  ...
  },
  {
    ...
    category: "Accessories",
    image: lichenImage,
    shortDesc: "Hand-brushed wool scarf coloured with natural moss pigment extracts.",
    longDesc:
      "Woven in Yorkshire on traditional looms, then gently brushed with hand-teasels to lift natural fluff. Coloured with chemical-free forest moss dyes, yielding deep melange sage and olive complexities.",
  ...
  },
  {
    ...
    longDesc:
      "A stunning heavy-weight knitted outer-layer designed to resist sea breeze. Modelled on standard fishermen garments, utilizing double-twisted wool to maximize rainwater deflection.",
