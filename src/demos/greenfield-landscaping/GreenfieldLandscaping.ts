...
function bindGreenfieldLandscaping(container: DemoContainer): void {
  ...
  container
    .querySelector("#greenfield-nav-cta")
    ?.addEventListener("click", handleSurveyCta);
  container
    .querySelector("#greenfield-hero-cta")
    ?.addEventListener("click", handleSurveyCta);
  container
    .querySelector("#greenfield-contact-form")
    ?.addEventListener("submit", handleSurveyCta);
}
...
export function renderGreenfieldLandscaping(container: DemoContainer): void {
  ...
  container.innerHTML = `
    <div class="bg-[#FAF9F5] min-h-screen overflow-x-hidden text-[#223322] font-sans antialiased text-left">
      ...
      <!-- Hero Section -->
      <section class="relative bg-stone-950 text-white py-20 md:py-32 overflow-hidden border-b border-[#2C5E3B]/10">
        ...
        <div class="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 space-y-6 text-left" data-reveal>
          ...
          <div class="flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:gap-4 pt-3">
            ...
            <a 
              href="#portfolio"
              class="w-full sm:w-auto bg-white/10 hover:border-white/40 hover:text-white hover:shadow-[0_0.85rem_2rem_rgba(15,23,42,0.12)] text-white border border-solid border-white/20 font-bold text-xs py-3.5 px-6 rounded transition-all text-center cursor-pointer"
            >
              View Completed Work
            </a>
          </div>
        ...
        <div class="max-w-xl mx-auto px-4 sm:px-6 space-y-6" data-reveal>
          ...
          <form id="greenfield-contact-form" class="space-y-3 p-6 bg-white border border-stone-200 rounded-lg text-left shadow-xs hover-lift transition-transform duration-300">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label for="greenfield-name" class="text-[10px] font-mono uppercase tracking-widest text-stone-500 font-bold block mb-1">Your Full Name</label>
                <input id="greenfield-name" name="name" type="text" required placeholder="Alistair Graham" class="w-full border border-stone-200 p-2.5 text-xs focus:outline-none focus:border-[#2C5E3B]" />
              </div>
              <div>
                <label for="greenfield-postcode" class="text-[10px] font-mono uppercase tracking-widest text-stone-500 font-bold block mb-1">North East Postcode</label>
                <input id="greenfield-postcode" name="postcode" type="text" required placeholder="e.g. NE65 1AA" class="w-full border border-stone-200 p-2.5 text-xs focus:outline-none focus:border-[#2C5E3B]" />
              </div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label for="greenfield-dimensions" class="text-[10px] font-mono uppercase tracking-widest text-stone-500 font-bold block mb-1">Garden Dimensions</label>
                <input id="greenfield-dimensions" name="dimensions" type="text" placeholder="e.g. 12m x 8m" class="w-full border border-stone-200 p-2.5 text-xs focus:outline-none focus:border-[#2C5E3B]" />
              </div>
              <div>
                <label for="greenfield-work-type" class="text-[10px] font-mono uppercase tracking-widest text-stone-500 font-bold block mb-1">Core Work Needed</label>
                <select id="greenfield-work-type" name="work-type" class="w-full border border-stone-200 p-2.5 text-xs text-stone-600 focus:outline-none bg-white">
            ...
            <div>
              <label for="greenfield-details" class="text-[10px] font-mono uppercase tracking-widest text-stone-500 font-bold block mb-1">Transform details</label>
              <textarea id="greenfield-details" name="details" rows="4" placeholder="Tell us about moss issues, water pooling in corners, or required sandstone paving outlines..." class="w-full border border-stone-200 p-2.5 text-xs focus:outline-none focus:border-[#2C5E3B] resize-none font-sans"></textarea>
            </div>

            <div>
              <label for="greenfield-photo" class="text-[10px] font-mono uppercase tracking-widest text-stone-500 font-bold block mb-1">Upload Site Photo (garden_weeds.jpg)</label>
              <input id="greenfield-photo" name="photo" type="file" accept="image/*" class="sr-only" />
              <label for="greenfield-photo" class="border border-dashed border-stone-300 rounded p-4 text-center cursor-pointer hover:border-[#2C5E3B]/40 hover:text-[#2C5E3B] hover:shadow-[0_0.85rem_2rem_rgba(15,23,42,0.06)] text-stone-400 flex flex-col items-center transition-all">
