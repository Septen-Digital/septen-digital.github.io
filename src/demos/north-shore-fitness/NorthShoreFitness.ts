...
function bindNorthShoreFitness(container: DemoContainer): void {
  ...
  container
    .querySelector("#fitness-nav-cta")
    ?.addEventListener("click", handleJoinCta);
  container
    .querySelector("#fitness-hero-cta")
    ?.addEventListener("click", handleJoinCta);
  container
    .querySelector("#fitness-membership-1")
    ?.addEventListener("click", handleJoinCta);
  container
    .querySelector("#fitness-membership-2")
    ?.addEventListener("click", handleJoinCta);
  container
    .querySelector("#fitness-contact-form")
    ?.addEventListener("submit", handleJoinCta);
}
      ...
      <section class="relative bg-[#0F1422] py-20 md:py-28 overflow-hidden border-b border-slate-800">
        ...
        <div class="relative z-10 max-w-4xl mx-auto px-6 space-y-6 text-left" data-reveal>
          ...
          <div class="flex flex-wrap gap-4 pt-3">
            ...
            <a 
              href="#timetables"
              class="bg-transparent hover:border-amber-500/60 hover:text-amber-300 hover:shadow-[0_0.85rem_2rem_rgba(245,158,11,0.12)] text-white border border-solid border-slate-705 font-bold text-xs py-3.5 px-6 rounded transition-all text-center cursor-pointer flex items-center justify-center"
            >
              View Class Times
            </a>
          </div>
      ...
      <section class="bg-[#0B0F19] py-16 text-center border-t border-slate-800">
        <div class="max-w-xl mx-auto px-6 space-y-6" data-reveal>
          ...
          <form id="fitness-contact-form" class="space-y-3.5 p-6 bg-[#0F1422] border border-slate-800 rounded-lg text-left shadow-md max-w-sm mx-auto hover-lift transition-transform duration-300 hover:border-amber-500/40">
            <div>
              <label for="fitness-name" class="text-[9px] font-mono uppercase tracking-widest text-slate-400 font-bold block mb-1">Your Full Name</label>
              <input id="fitness-name" name="name" type="text" required placeholder="Marcus Vance" class="w-full bg-[#121829] border border-slate-800 text-white rounded p-2.5 text-xs focus:outline-none focus:border-amber-500" />
            </div>
            <div>
              <label for="fitness-email" class="text-[9px] font-mono uppercase tracking-widest text-slate-400 font-bold block mb-1">Email Address</label>
              <input id="fitness-email" name="email" type="email" required placeholder="marcus@vance.com" class="w-full bg-[#121829] border border-slate-800 text-white rounded p-2.5 text-xs focus:outline-none focus:border-amber-500" />
            </div>
            <div>
              <label for="fitness-goal" class="text-[9px] font-mono uppercase tracking-widest text-slate-400 font-bold block mb-1">Core Training Goal</label>
              <select id="fitness-goal" name="goal" class="w-full bg-[#121829] border border-slate-800 text-slate-300 rounded p-2.5 text-xs focus:outline-none focus:border-amber-500">
