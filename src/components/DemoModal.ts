...
export function openDemoModal(businessName = 'this local business'): void {
  ...
  container.className = 'fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in';
  ...
  container.innerHTML = `
    <div id="demo-modal-card" class="bg-white rounded-xl shadow-2xl border border-slate-100 max-w-md w-full p-6 text-left relative overflow-hidden animate-slide-in-up" role="document">
      ...
      <button 
        ...
        class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 p-1.5 rounded-lg hover:bg-slate-50 transition-colors border-none cursor-pointer"
