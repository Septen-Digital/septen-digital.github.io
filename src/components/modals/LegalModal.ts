...
export function openLegalModal(tab: LegalTab = "terms", options: LegalModalOptions = {}): void {
  ...
  const tabsMarkup = legalTabs
    .map(
      (item: { id: LegalTab; label: string }) =>
        `<button type="button" data-tab="${item.id}" role="tab" aria-controls="legal-panel-${item.id}" aria-selected="false" class="shrink-0 px-4 sm:px-6 py-3 text-[0.8125rem] font-extrabold uppercase tracking-widest text-slate-500 border-b-2 border-transparent hover:border-slate-300 hover:text-slate-700 transition-colors bg-transparent cursor-pointer whitespace-nowrap">${item.label}</button>`,
    )
    .join("");
  ...
  container.innerHTML = `
    <div class="bg-white rounded-2xl shadow-2xl border border-slate-100 max-w-4xl w-full max-h-[min(88dvh,90vh)] h-[min(88dvh,90vh)] flex flex-col min-h-0 relative overflow-hidden animate-slide-in-up" role="document">
      <button id="legal-modal-close-btn" class="absolute top-3 right-3 w-11 h-11 flex items-center justify-center rounded-full bg-brand-orange hover:bg-brand-orange-dark text-white hover:text-white transition-colors border-2 border-brand-orange hover:border-brand-orange-dark shadow-[0_10px_24px_rgba(175,98,69,0.28)] hover:shadow-[0_14px_32px_rgba(138,74,53,0.34)] cursor-pointer z-20 focus:outline-none focus:ring-2 focus:ring-brand-orange focus:ring-offset-2" aria-label="Close modal">
        ${getIcon("X", "w-6 h-6 cursor-pointer")}
      </button>
      <div class="px-4 sm:px-6 py-4 sm:py-5 border-b border-slate-100 shrink-0 pr-20">
        <span class="font-sans font-black text-[0.6875rem] uppercase text-brand-teal tracking-widest block mb-1.5">Legal</span>
  ...
  const setTab = (activeTab: LegalTab, behavior: ScrollBehavior = "auto"): void => {
    ...
    tabButtons.forEach((button) => {
      ...
      button.className = isActive
        ? "shrink-0 px-4 sm:px-6 py-3 text-[0.8125rem] font-extrabold uppercase tracking-widest border-b-2 border-brand-teal text-brand-teal bg-transparent cursor-pointer whitespace-nowrap"
        : "shrink-0 px-4 sm:px-6 py-3 text-[0.8125rem] font-extrabold uppercase tracking-widest text-slate-500 border-b-2 border-transparent hover:border-slate-300 hover:text-slate-700 transition-colors bg-transparent cursor-pointer whitespace-nowrap";
    });
